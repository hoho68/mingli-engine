...
from mingli_engine.models import (
    ...
    NewMaterialExpandedCorrectedTranscriptionExecutionItem,
    NewMaterialExpandedCorrectedTranscriptionExecutionSummary,
...
NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_ID = "015-new-material-source-identity-review"
NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_NEXT_MATERIAL_ENTRY = (
    "015-new-material-registration-prep"
)
NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_STATUSES = frozenset(
    {"identity_review_completed"}
)
NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_OVERLAP_STATUSES = frozenset(
...
NEW_MATERIAL_OCR_RUNTIME_SETUP_ID = (
    "015-new-material-ocr-runtime-setup-or-human-transcription"
)
...
NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_ID = (
    "015-new-material-expanded-corrected-transcription-execution"
)
NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_NEXT_MATERIAL_ENTRY = (
...
NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_STATUSES = frozenset(
    {"expanded_prepared_text_created"}
)
...
def _new_material_source_identity_review_item_from_dict(
    data: dict[str, Any],
    source_dir: Path,
) -> NewMaterialSourceIdentityReviewItem:
    try:
        item = NewMaterialSourceIdentityReviewItem(**data)
    except TypeError as error:
        raise MaterialsAuditError(
            f"invalid new material source identity review item: {error}"
        ) from error
    ...
    _validate_enum(
        item.identity_status,
        NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_STATUSES,
        "identity_status",
        owner_id,
    )
    _validate_enum(
        item.source_library_overlap_status,
        NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_OVERLAP_STATUSES,
        "source_library_overlap_status",
        owner_id,
    )
    _validate_enum(
        item.registration_readiness,
        NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_REGISTRATION_READINESS,
        "registration_readiness",
        owner_id,
    )
    _validate_enum(
    ...
    _require_string_list(
        item.matched_source_library_entry_ids,
        "matched_source_library_entry_ids",
        owner_id,
    )
    _require_string_list(item.target_rule_families, "target_rule_families", owner_id)
    ...
    if item.file_count != len(item.relative_paths):
        raise MaterialsAuditError(f"{owner_id} file_count must match relative_paths")
    if item.file_count != 1 or item.priority_text_candidate_count != 1:
        raise MaterialsAuditError(f"{owner_id} must remain a single-file review")
    for field_name in (
        "file_count",
        "priority_text_candidate_count",
        "candidate_extract_delta_count",
    ...
    intake_items_by_id = {
        intake.intake_item_id: intake
        for intake in load_new_material_intake_items(source_dir)
    }
    intake = intake_items_by_id.get(item.intake_item_id)
    if intake is None:
        raise MaterialsAuditError(f"{owner_id} references unknown intake_item_id")
    if intake.intake_status != "selected_for_source_identity_review":
        raise MaterialsAuditError(f"{owner_id} intake item is not ready")
    if item.cluster_id != intake.cluster_id:
        raise MaterialsAuditError(f"{owner_id} intake cluster mismatch")
    if item.source_selection_id != intake.source_selection_id:
        raise MaterialsAuditError(f"{owner_id} intake source-selection mismatch")
    if item.source_root != intake.source_root:
        raise MaterialsAuditError(f"{owner_id} intake source_root mismatch")
    if item.risk_boundary != intake.risk_boundary:
        raise MaterialsAuditError(f"{owner_id} intake risk boundary mismatch")
    if item.relative_paths != intake.relative_paths:
        raise MaterialsAuditError(f"{owner_id} relative_paths must match intake")
    if set(item.target_rule_families) != set(intake.target_rule_families):
        raise MaterialsAuditError(f"{owner_id} rule families must match intake")
    if not all(_is_source_relative_path(path) for path in item.relative_paths):
        raise MaterialsAuditError(f"{owner_id} relative_paths must stay relative")
    ...
    if missing_entry_ids:
        raise MaterialsAuditError(f"{owner_id} references unknown source-library ids")
    registered_overlap_ids = [
        entry.entry_id
    ...
    if (
        ...
        and registered_overlap_ids
    ):
        raise MaterialsAuditError(f"{owner_id} has unexpected source-library overlap")
    if item.matched_source_library_entry_ids != registered_overlap_ids:
        raise MaterialsAuditError(f"{owner_id} source-library overlap ids mismatch")
    return item


def load_new_material_source_identity_review_items(
    data_dir: Path | str | None = None,
) -> list[NewMaterialSourceIdentityReviewItem]:
    source_dir = _data_dir(data_dir)
    items_path = source_dir / "new_material_source_identity_review_items.json"
    if not items_path.exists():
        return []
    items = [
        _new_material_source_identity_review_item_from_dict(item, source_dir)
        for item in _read_optional_json_list(items_path)
    ]
    _ensure_unique([item.review_item_id for item in items], "review_item_id")
    return items
...
def build_new_material_source_identity_review_summary(
    data_dir: Path | str | None = None,
) -> NewMaterialSourceIdentityReviewSummary:
    source_dir = _data_dir(data_dir)
    items = load_new_material_source_identity_review_items(source_dir)
    intake_items_by_id = {
        item.intake_item_id: item for item in load_new_material_intake_items(source_dir)
    }
    source_entries_by_id = _load_source_library_entries(source_dir)
    intake_references_valid = bool(items) and all(
        item.intake_item_id in intake_items_by_id for item in items
    )
    intake_paths_match = bool(items) and all(
        item.intake_item_id in intake_items_by_id
        and item.relative_paths == intake_items_by_id[item.intake_item_id].relative_paths
        for item in items
    )
    ...
    no_registered_overlap_found = bool(items) and all(
        not any(
    ...
    boundary_checks = {
        "identity_review_items_loaded": "passed" if items else "failed",
        "intake_references_valid": (
            "passed" if intake_references_valid else "failed"
        ),
        "intake_paths_match": "passed" if intake_paths_match else "failed",
        "selected_paths_are_relative": (
            "passed" if selected_paths_are_relative else "failed"
        ),
        "single_file_boundary": (
        ...
        "no_registered_source_library_overlap": (
            "passed" if no_registered_overlap_found else "failed"
        ),
        "source_library_not_mutated": (
            "passed"
            if not any(item.source_library_mutation_authorized for item in items)
    ...
    return NewMaterialSourceIdentityReviewSummary(
        review_id=NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_ID,
        review_status=(
            "identity_review_completed"
            if all(status == "passed" for status in boundary_checks.values())
            else "identity_review_needs_attention"
        ),
        review_item_count=len(items),
        identity_completed_count=sum(
            1 for item in items if item.identity_status == "identity_review_completed"
        ),
        registration_prep_ready_count=sum(
            1
            for item in items
            if item.registration_readiness == "ready_for_registration_prep"
        ),
        source_library_overlap_found_count=sum(
            1 for item in items if item.matched_source_library_entry_ids
        ),
        source_file_count=sum(item.file_count for item in items),
        priority_text_candidate_count=sum(
            item.priority_text_candidate_count for item in items
        ...
        next_material_entry=NEW_MATERIAL_SOURCE_IDENTITY_REVIEW_NEXT_MATERIAL_ENTRY,
        review_item_ids=[item.review_item_id for item in items],
        intake_item_ids=[item.intake_item_id for item in items],
        cluster_ids=[item.cluster_id for item in items],
        source_selection_ids=[item.source_selection_id for item in items],
        canonical_title_labels=[item.canonical_title_label for item in items],
        relative_paths=[path for item in items for path in item.relative_paths],
        target_rule_family_counts=_count_values(
            [family for item in items for family in item.target_rule_families]
        ),
...
def render_new_material_source_identity_review_markdown(
    summary: NewMaterialSourceIdentityReviewSummary,
) -> str:
    source_library_mutation_authorized = (
        "true" if summary.source_library_mutation_authorized else "false"
    )
    ...
    lines = [
        ...
        "",
        f"- Review id: `{summary.review_id}`",
        f"- `new-material-source-identity-review-status={summary.review_status}`",
        f"- `identity-review-items={summary.review_item_count}`",
        f"- `identity-completed={summary.identity_completed_count}`",
        f"- `registration-prep-ready={summary.registration_prep_ready_count}`",
        (
            "- `source-library-overlap-found="
            f"{summary.source_library_overlap_found_count}`"
        ),
        f"- `reviewed-source-files={summary.source_file_count}`",
        (
            "- `reviewed-priority-candidates="
            f"{summary.priority_text_candidate_count}`"
        ),
        f"- `candidate-extract-delta={summary.candidate_extract_delta_count}`",
        f"- `review-decision-delta={summary.review_decision_delta_count}`",
    ...
    lines.extend(f"- `{item_id}`" for item_id in summary.review_item_ids)
    lines.extend(["", "Intake item ids:"])
    lines.extend(f"- `{item_id}`" for item_id in summary.intake_item_ids)
    lines.extend(["", "Canonical title labels:"])
    lines.extend(f"- `{title}`" for title in summary.canonical_title_labels)
    lines.extend(["", "Relative paths:"])
    lines.extend(f"- `{path}`" for path in summary.relative_paths)
    lines.extend(["", "Boundary checks:"])
    lines.extend(
...
def _new_material_ocr_runtime_setup_item_from_dict(
    ...
    try:
        ...
        raise MaterialsAuditError(
            f"invalid new material OCR runtime setup item: {error}"
        ) from error
    owner_id = item.setup_item_id or "?"
    for field_name in (
        "setup_item_id",
        "setup_id",
        "ocr_or_manual_transcription_item_id",
        "source_library_entry_id",
        "source_material_id",
        "setup_status",
        "selected_path",
        ...
        _require_text(getattr(item, field_name), field_name, owner_id)
    if item.setup_id != NEW_MATERIAL_OCR_RUNTIME_SETUP_ID:
        raise MaterialsAuditError(f"{owner_id} has invalid setup_id")
    if item.selected_next_material_entry != (
        NEW_MATERIAL_OCR_RUNTIME_SETUP_NEXT_MATERIAL_ENTRY
    ):
        raise MaterialsAuditError(f"{owner_id} selected unexpected next entry")
    _validate_enum(
        item.setup_status,
        NEW_MATERIAL_OCR_RUNTIME_SETUP_STATUSES,
        "setup_status",
        owner_id,
    )
    if item.selected_path != "local_tesseract_quality_probe":
        raise MaterialsAuditError(f"{owner_id} has invalid selected_path")
    _require_string_list(item.tessdata_language_codes, "tessdata_language_codes", owner_id)
    _require_string_list(item.probe_psm_values, "probe_psm_values", owner_id)
    _require_string_list(item.guardrails, "guardrails", owner_id)
    for field_name in (
        "page_count",
        "probe_page_count",
    ...
    if item.page_count <= 0 or item.probe_page_count <= 0 or item.probe_dpi <= 0:
        raise MaterialsAuditError(f"{owner_id} has invalid OCR probe counts")
    if not item.pdftoppm_available:
        raise MaterialsAuditError(f"{owner_id} must have PDF rendering available")
    if not item.tesseract_available or not item.chi_sim_available:
        raise MaterialsAuditError(f"{owner_id} must have local Tesseract Chinese OCR")
    if "chi_sim" not in item.tessdata_language_codes:
        raise MaterialsAuditError(f"{owner_id} missing chi_sim tessdata")
    ...
    if previous is None:
        raise MaterialsAuditError(
            f"{owner_id} references unknown OCR/manual transcription item"
        )
    if previous.transcription_status != "blocked_ocr_runtime_unavailable":
        raise MaterialsAuditError(f"{owner_id} previous OCR/manual item not blocked")
    if item.source_library_entry_id != previous.source_library_entry_id:
        raise MaterialsAuditError(f"{owner_id} source entry mismatch")
    if item.source_material_id != previous.source_material_id:
        raise MaterialsAuditError(f"{owner_id} source material mismatch")
    if item.local_reference != previous.local_reference:
        raise MaterialsAuditError(f"{owner_id} local reference mismatch")
    if item.page_count != previous.page_count:
        raise MaterialsAuditError(f"{owner_id} page count mismatch")
    return item


def load_new_material_ocr_runtime_setup_items(
    data_dir: Path | str | None = None,
...
def build_new_material_ocr_runtime_setup_summary(
    ...
    boundary_checks = {
        ...
        "pdf_rendering_available": (
            "passed" if all(item.pdftoppm_available for item in items) else "failed"
        ),
        "local_tesseract_available": (
            "passed" if all(item.tesseract_available for item in items) else "failed"
        ),
        "chi_sim_tessdata_available": (
            "passed" if all(item.chi_sim_available for item in items) else "failed"
        ),
        "prepared_text_artifact_absent": (
            "passed"
            if all(not item.prepared_text_artifact_created for item in items)
    ...
    return NewMaterialOcrRuntimeSetupSummary(
        setup_id=NEW_MATERIAL_OCR_RUNTIME_SETUP_ID,
        setup_status=(
            "blocked_ocr_quality_insufficient"
            if all(status == "passed" for status in boundary_checks.values())
            else "ocr_runtime_setup_needs_attention"
        ),
        setup_item_count=len(items),
        source_file_count=len(items),
        page_count=sum(item.page_count for item in items),
        probe_page_count=sum(item.probe_page_count for item in items),
        probe_dpi_values=sorted({item.probe_dpi for item in items}),
        pdftoppm_available_count=sum(1 for item in items if item.pdftoppm_available),
        tesseract_available_count=sum(1 for item in items if item.tesseract_available),
        chi_sim_available_count=sum(1 for item in items if item.chi_sim_available),
        prepared_text_artifact_count=sum(
            1 for item in items if item.prepared_text_artifact_created
        ),
        blocked_item_count=sum(
            1
            for item in items
            if item.setup_status == "blocked_ocr_quality_insufficient"
        ),
        setup_item_ids=[item.setup_item_id for item in items],
        ocr_or_manual_transcription_item_ids=[
            item.ocr_or_manual_transcription_item_id for item in items
        ],
        source_entry_ids=[item.source_library_entry_id for item in items],
        source_material_ids=[item.source_material_id for item in items],
        local_references=[item.local_reference for item in items],
        ...
        next_material_entry=NEW_MATERIAL_OCR_RUNTIME_SETUP_NEXT_MATERIAL_ENTRY,
        boundary_checks=boundary_checks,
        guardrails=[
...
def render_new_material_ocr_runtime_setup_markdown(
    summary: NewMaterialOcrRuntimeSetupSummary,
) -> str:
    source_library_mutation_authorized = (
        "true" if summary.source_library_mutation_authorized else "false"
    )
    ...
    lines = [
        ...
        f"- `new-material-ocr-runtime-setup-status={summary.setup_status}`",
        f"- `ocr-runtime-setup-items={summary.setup_item_count}`",
        f"- `source-files={summary.source_file_count}`",
        f"- `pdf-pages={summary.page_count}`",
        f"- `probe-pages={summary.probe_page_count}`",
        f"- `probe-dpi-values={probe_dpi_values}`",
        f"- `pdftoppm-available={summary.pdftoppm_available_count}`",
        f"- `tesseract-available={summary.tesseract_available_count}`",
        f"- `chi-sim-available={summary.chi_sim_available_count}`",
        f"- `prepared-text-artifacts={summary.prepared_text_artifact_count}`",
        f"- `blocked-items={summary.blocked_item_count}`",
        f"- `candidate-extract-delta={summary.candidate_extract_delta_count}`",
        f"- `formal-evidence-delta={summary.formal_evidence_delta_count}`",
    ...
    lines.extend(f"- `{item_id}`" for item_id in summary.setup_item_ids)
    lines.extend(["", "Previous OCR/manual transcription item ids:"])
    lines.extend(
        f"- `{item_id}`" for item_id in summary.ocr_or_manual_transcription_item_ids
    )
    lines.extend(["", "Source-library entry ids:"])
    lines.extend(f"- `{entry_id}`" for entry_id in summary.source_entry_ids)
    lines.extend(["", "Local references:"])
...
def _new_material_expanded_corrected_transcription_execution_item_from_dict(
    data: dict[str, Any],
    source_dir: Path,
) -> NewMaterialExpandedCorrectedTranscriptionExecutionItem:
    try:
        item = NewMaterialExpandedCorrectedTranscriptionExecutionItem(**data)
    except TypeError as error:
        raise MaterialsAuditError(
            "invalid new material expanded corrected transcription execution "
            f"item: {error}"
        ) from error
    owner_id = item.execution_item_id or "?"
    for field_name in (
        ...
        _require_text(getattr(item, field_name), field_name, owner_id)
    if item.execution_id != NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_ID:
        raise MaterialsAuditError(f"{owner_id} has invalid execution_id")
    if item.selected_next_material_entry != (
        NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_NEXT_MATERIAL_ENTRY
    ):
    ...
    _validate_enum(
        item.execution_status,
        NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_STATUSES,
        "execution_status",
        owner_id,
    )
    _require_string_list(item.selected_page_ranges, "selected_page_ranges", owner_id)
    ...
    for field_name in (
        ...
        _require_non_negative_int(getattr(item, field_name), field_name, owner_id)
    if item.selected_page_count <= 0 or item.expanded_window_count <= 0:
        raise MaterialsAuditError(f"{owner_id} must contain expanded windows")
    if item.corrected_excerpt_count <= 0 or item.corrected_character_count <= 0:
        raise MaterialsAuditError(f"{owner_id} must contain corrected excerpts")
    if item.corrected_character_count > 180:
        raise MaterialsAuditError(f"{owner_id} corrected expanded text is too long")
    if item.page_locator_count != len(item.selected_page_locators):
        raise MaterialsAuditError(f"{owner_id} page locator count mismatch")
    if item.expanded_window_count != len(item.selected_page_ranges):
        raise MaterialsAuditError(f"{owner_id} expanded window count mismatch")
    if item.uncorrected_ocr_committed or item.long_form_transcription_committed:
        raise MaterialsAuditError(f"{owner_id} violates OCR text boundary")
    if not item.prepared_text_artifact_created:
        raise MaterialsAuditError(f"{owner_id} must create prepared text artifact")
    ...
    prep_items_by_id = {
        prep.prep_item_id: prep
        for prep in load_new_material_expanded_corrected_transcription_prep_items(
            source_dir
        )
    }
    prep = prep_items_by_id.get(item.prep_item_id)
    if prep is None:
        raise MaterialsAuditError(f"{owner_id} references unknown prep item")
    if prep.prep_status != "ready_for_expanded_correction_execution":
        raise MaterialsAuditError(f"{owner_id} prep item not ready for execution")
    matching_fields = (
        "source_library_entry_id",
        "source_material_id",
        "local_reference",
    ...
    for field_name in matching_fields:
        if getattr(item, field_name) != getattr(prep, field_name):
            raise MaterialsAuditError(f"{owner_id} {field_name} mismatch")
    if item.prepared_text_artifact != prep.planned_output_artifact:
        raise MaterialsAuditError(f"{owner_id} prepared text artifact mismatch")
    if set(item.selected_page_ranges) != set(prep.selected_page_ranges):
        raise MaterialsAuditError(f"{owner_id} selected page ranges mismatch")
    if set(item.selected_page_locators) != set(prep.selected_page_locators):
        raise MaterialsAuditError(f"{owner_id} selected page locators mismatch")
    return item


def load_new_material_expanded_corrected_transcription_execution_items(
    data_dir: Path | str | None = None,
) -> list[NewMaterialExpandedCorrectedTranscriptionExecutionItem]:
    source_dir = _data_dir(data_dir)
    items_path = (
        source_dir
        / "new_material_expanded_corrected_transcription_execution_items.json"
    )
    if not items_path.exists():
        return []
    items = [
        _new_material_expanded_corrected_transcription_execution_item_from_dict(
            item,
            source_dir,
        )
        for item in _read_optional_json_list(items_path)
...
def build_new_material_expanded_corrected_transcription_execution_summary(
    data_dir: Path | str | None = None,
) -> NewMaterialExpandedCorrectedTranscriptionExecutionSummary:
    source_dir = _data_dir(data_dir)
    ...
    boundary_checks = {
        "expanded_corrected_transcription_execution_items_loaded": (
            "passed" if items else "failed"
        ),
        ...
        "corrected_text_available": (
            "passed"
            if all(item.human_corrected_text_available for item in items)
            else "failed"
        ),
        "uncorrected_ocr_not_committed": (
            "passed"
    ...
    return NewMaterialExpandedCorrectedTranscriptionExecutionSummary(
        execution_id=NEW_MATERIAL_EXPANDED_CORRECTED_TRANSCRIPTION_EXECUTION_ID,
        execution_status=(
            "expanded_prepared_text_created"
            if all(status == "passed" for status in boundary_checks.values())
            else "expanded_corrected_transcription_execution_needs_attention"
        ),
        execution_item_count=len(items),
        source_file_count=len(items),
        prepared_text_artifact_count=sum(
        ...
        selected_page_count=sum(item.selected_page_count for item in items),
        expanded_window_count=sum(item.expanded_window_count for item in items),
        corrected_excerpt_count=sum(item.corrected_excerpt_count for item in items),
        corrected_character_count=sum(item.corrected_character_count for item in items),
        page_locator_count=sum(item.page_locator_count for item in items),
        learning_entry_ready_count=sum(1 for item in items if item.learning_entry_ready),
...
def render_new_material_expanded_corrected_transcription_execution_markdown(
    summary: NewMaterialExpandedCorrectedTranscriptionExecutionSummary,
) -> str:
    downstream_mutation_authorized = (
        "true" if summary.downstream_mutation_authorized else "false"
    )
    lines = [
        ...
        "",
        f"- Execution id: `{summary.execution_id}`",
        f"- `new-material-expanded-corrected-transcription-execution-status={summary.execution_status}`",
        f"- `expanded-correction-execution-items={summary.execution_item_count}`",
        f"- `source-files={summary.source_file_count}`",
        f"- `prepared-text-artifacts={summary.prepared_text_artifact_count}`",
        f"- `selected-page-ranges={summary.selected_page_range_count}`",
        f"- `selected-page-locators={summary.selected_page_locator_count}`",
        f"- `selected-pages={summary.selected_page_count}`",
        f"- `expanded-windows={summary.expanded_window_count}`",
        f"- `corrected-excerpts={summary.corrected_excerpt_count}`",
        f"- `corrected-characters={summary.corrected_character_count}`",
        f"- `page-locators={summary.page_locator_count}`",
        f"- `learning-entry-ready={summary.learning_entry_ready_count}`",
    ...
    lines.extend(f"- `{item_id}`" for item_id in summary.execution_item_ids)
    lines.extend(["", "Expanded correction prep item ids:"])
    lines.extend(f"- `{item_id}`" for item_id in summary.prep_item_ids)
    lines.extend(["", "Prepared text artifacts:"])
    lines.extend(f"- `{artifact}`" for artifact in summary.prepared_text_artifacts)
    lines.extend(["", "Local references:"])
...
def validate_materials_audit_quality(data_dir: Path | str | None = None) -> list[str]:
    try:
        ...
        new_material_expanded_corrected_transcription_execution_items = (
...
def _iter_quality_text_fields(
    ...
    new_material_expanded_corrected_transcription_execution_items: list[
        NewMaterialExpandedCorrectedTranscriptionExecutionItem
    ],
    ...
    for item in new_material_source_identity_review_items:
        fields.extend(
            (
                (item.review_item_id, "source_label", item.source_label),
                (
                    item.review_item_id,
                    "canonical_title_label",
    ...
    for item in new_material_ocr_runtime_setup_items:
        fields.extend(
            (
                (item.setup_item_id, "local_reference", item.local_reference),
                (item.setup_item_id, "blocker_reason", item.blocker_reason),
                (item.setup_item_id, "rationale", item.rationale),
            )
        )
        fields.extend(
    ...
    for item in new_material_expanded_corrected_transcription_execution_items:
        fields.extend(
            (
                (item.execution_item_id, "local_reference", item.local_reference),
                (
        ...
        fields.extend(
            (item.execution_item_id, "selected_page_ranges", page_range)
            for page_range in item.selected_page_ranges
        )
        fields.extend(
            (item.execution_item_id, "selected_page_locators", locator)
