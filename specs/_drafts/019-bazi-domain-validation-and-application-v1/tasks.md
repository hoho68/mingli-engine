...
- [ ] T012 Write failing exact-field, literal, tuple, immutability, `ExactVersionSet`, and complete response-matrix tests in `tests/unit/test_application_models.py`
...
- [ ] T071 Freeze the exact equal run/baseline/release `ExactVersionSet`, corpus hashes, metrics, and claim boundary in `src/mingli_engine/data/domain_calibration/calibration_baseline.json`, and verify read-only loading and equality in `tests/integration/test_domain_calibration_pipeline.py`
