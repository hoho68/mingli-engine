...
- Risk tier: `ordinary`
- Summary: 刑冲合害须结合位置、引动条件与整体结构观察。
- Limitation: 没有岁运触发时只作为结构压力或互动线索。

## Source Windows

### source-window-branch-interactions
...
### source-window-branch-interaction-position
...
- Limitation: 缺少位置和十神时只作为互动线索。

### source-window-branch-interaction-trigger
...
- Limitation: 没有岁运触发时不得推具体应事。

### source-window-branch-interaction-strength
...
- Limitation: 合化条件不明时保留为候选。

### source-window-blind-school-image
...
- Limitation: 无互证时不得作为正式判断。

### source-window-blind-school-image-boundary
...
- Limitation: 不得输出无法追溯的断语。

### source-window-blind-high-risk-boundary
...
- Limitation: 拒绝精确灾期、疾病诊断、治疗方案和保证结果。

### source-window-blind-remedy-boundary
...
- Limitation: 不得制造焦虑或引导付费化解。

## Remaining Curation

- Add page or chapter anchors after a readable extract is reviewed.
- Separate branch interaction, image-method, and high-risk language into
  independently reviewable cards.
