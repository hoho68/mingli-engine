...
prep, and raw text next-cycle gated ordinary source selection, and raw text next-cycle gated ordinary followup selection.
The latest local checkpoint selected the second pair of bounded ordinary
source-level records from the prepared case/formula gated clusters, registered
...
- Candidate/formal evidence boundary audit confirms 017 remains provenance
  metadata while authorized downstream Bazi general records are represented in
  013 and 012: `formal_evidence_delta=0`.
...
- 015 coverage-aware queue refresh excludes the 29 queue ids already covered by
...
- 015 raw text materials folder risk triage split `资料原文/文本类/` into 11
  ...
  `deferred-groups=6`.
- 015 Liang Bazi Core Individual Review is completed for
...
- 015 Bazi General Source Identity Review is completed:
  ...
  mutation was authorized.
- 015 Bazi General Registration Prep is completed:
  `registration-prep-items=3`, `proposed-source-files=4`,
...
- 015 Bazi General Source Preparation Reading is completed:
...
- 015 Raw Text Next Cycle Source Selection is completed:
  `next-cycle-source-selection-status=next_cycle_source_selection_completed`,
  `next-cycle-source-selection-items=5`, `selected-for-identity-review=2`,
...
- 015 Raw Text Next Cycle Followup Selection is completed:
...
- 015 Raw Text Next Cycle Gated Ordinary Followup Selection is completed:
...
Deferred or boundary-gated next-cycle clusters:
...
- Selected individual review ids are now completed:
  `liang_tianyuan_wuxian_commentary` and
  `liang_yushi_yongshen_ciyuan`.
- 015 Liang Bazi Core Individual Review is completed:
...
- 015 Raw Text Next Cycle Source Selection is completed:
  `next-cycle-source-selection-status=next_cycle_source_selection_completed`,
  `next-cycle-source-selection-items=5`,
  `selected-for-identity-review=2`, `deferred-clusters=2`,
  `risk-review-clusters=1`, `downstream-mutation-authorized=false`, and
  `next-material-entry=015-raw-text-next-cycle-identity-review`.
...
- 015 Bazi General Registration Prep is completed:
  ...
  `registration-prep-items=3`, `proposed-source-files=4`,
  `skipped-existing-batch-overlap=2`, `blocked-variant-choice=2`,
  `deferred-large-source=1`, `source-library-mutation-authorized=false`,
  `downstream-mutation-authorized=false`, and
  `next-material-entry=015-bazi-general-source-registration`.
...
- Registration-prep record ids:
  `bazi_general_registration_prep_lecture_textbook`,
  `bazi_general_registration_prep_beichen_intro`, and
  `bazi_general_registration_prep_ziping_orthodox_pair`.
- Existing Batch 001 overlap ids remain skipped:
  `bazi_general_identity_youran_notes` and
  `bazi_general_identity_tianma_notes`.
- Variant-choice and deferred ids remain outside source registration:
...
- 015 Bazi General Source Preparation Reading is completed:
  ...
  `formal-evidence-units=3`, `downstream-mutation-authorized=true`, and
  `next-material-entry=015-bazi-general-variant-choice-and-deferred-review`.
- Promoted Bazi general candidate ids:
  `candidate_bazi_general_lecture_pattern_strength_001`,
  `candidate_bazi_general_beichen_branch_interaction_001`, and
  `candidate_bazi_general_ziping_useful_god_001`.
- Bazi general evidence ids:
...
- Sensitive prep created no source-library, 013 candidate, or 012 evidence
  records.
- 015 Raw Text Next Cycle Sensitive Source-Level Risk Review is completed:
...
- Sensitive source-level review id:
  `sensitive_source_review_bazi_psychology_pdf`.
- Reviewed sensitive prep id:
  `sensitive_risk_prep_bazi_psychology_pdf`.
...
- 015 Raw Text Next Cycle Sensitive Source Registration is completed:
...
- Registered sensitive source-library ids:
  `entry_bazi_general_bazi_psychology_pdf` and
  `material_bazi_general_bazi_psychology_pdf`.
...
- 015 New Material Expanded Corrected Transcription Execution is completed:
  `new-material-expanded-corrected-transcription-execution-status=expanded_prepared_text_created`,
  `expanded-correction-execution-items=1`, `selected-page-ranges=2`,
  `selected-page-locators=4`, `selected-pages=15`,
  `expanded-windows=2`, `corrected-excerpts=3`,
  `corrected-characters=27`, `prepared-text-artifacts=1`,
  `learning-entry-ready=1`, `human-corrected-text-available=1`,
  `uncorrected-ocr-committed=0`, `long-form-transcription-committed=0`,
  `candidate-intake-allowed=0`, `candidate-extract-delta=0`,
  `review-decision-delta=0`, `promotion-batch-delta=0`,
  `formal-evidence-delta=0`, `downstream-mutation-authorized=false`, and
  `next-material-entry=017-new-material-expanded-corrected-learning-entry-evaluation`.
- Expanded corrected transcription execution item:
