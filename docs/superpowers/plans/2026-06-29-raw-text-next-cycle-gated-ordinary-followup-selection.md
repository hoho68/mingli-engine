# Raw Text Next Cycle Gated Ordinary Followup Selection Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Select the next bounded ordinary source-level records from the prepared case-collection and practical-formula gated clusters, then register their weak-location 015/016/017/013/012 metadata without touching external raw materials.

**Architecture:** Add a gated ordinary followup selection layer that references both the original gated prep records and the first ordinary source-selection records. The followup selected records become source-library entries, materials-audit records, extraction queue tasks, learning reference notes, promoted 013 candidates, and formal 012 evidence with weak source-file/page locators.
...
- [x] Add tests for `load_raw_text_next_cycle_gated_ordinary_followup_selection_items`, summary counts, markdown/docs sync, selected source ids, prior-selection exclusion, sensitive-cluster exclusion, source-library mutation authorization, 013 candidate promotion, and 012 evidence promotion.
...
- Create: `src/mingli_engine/data/materials_audit/raw_text_next_cycle_gated_ordinary_followup_selection_items.json`

- [x] Add gated ordinary followup selection dataclasses.
- [x] Add constants, loader validation, summary builder, markdown renderer, and quality-gate integration.
- [ ] Store exactly two followup selected ordinary records: `八字18本/八字百诀 上册  214P.pdf` and `命理秘诀(1).pdf`.
- [x] Verify the first ordinary selected records are not duplicated and the sensitive topic prep item remains risk-review only.
...
- [x] Add the rendered 015 gated ordinary followup source-selection section.
...
- [x] Commit locally with `feat: select gated ordinary followup raw text sources`.
